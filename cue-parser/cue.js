...
function parseIndex(params, cueSheet) {
  ...
  if (!track.indexes) {
    if (_number > 2) {
      throw new Error(
        `Invalid index number ${_number}, First index number must be 0 or 1`
      );
    }
    track.indexes = [];
  } else {
    if (_number !== track.indexes[track.indexes.length - 1].number + 1) {
      throw new Error(
        `Invalid index number: ${_number}, it should follow the last sequence`
      );
    }
  }
...
function parseTime(timeSting) {
  ...
  if (time.frame > 99) {
    throw new Error(`Time frame should be less than 99: ${timeSting}`);
  }
  return time;
}
